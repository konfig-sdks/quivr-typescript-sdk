...
import { readableStreamToString, QuivrError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new QuivrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new QuivrError(e, e.response?.data)
                }
                if (innerError instanceof QuivrError) {
                    // Got "QuivrError" from the above try block
                    throw innerError;
                }
